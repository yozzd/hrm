...
export default {
      ...
      columns: [
        ...
        { title: 'Tempat Lahir', key: 'tempatLahir', width: 180 },
        {
          ...
          render: (h, params) => {
            return h('div', [
              h(
                'span',
                moment(new Date(params.row.tanggalLahir)).format('DD-MM-YYYY'),
              ),
            ]);
          },
        },
        {
          ...
          render: (h, params) => {
            ...
            return h('div', [h('span', p[params.row.pendidikan])]);
          },
        },
  ...
  computed: {
    formOption() {
      const alamat = _.uniq([
        this.karyawanDetail.personal.alamatSekarang,
        this.karyawanDetail.personal.alamatKTP,
      ]);
      return {
        forms: [
          {
          ...
          {
            prop: 'hubunganKeluarga',
            label: 'Hubungan Keluarga',
            itemType: 'select',
            options: hubunganKeluarga.options,
            filterable: true,
            rules: [
              {
                required: true,
                message: 'Pilih Hubungan Keluarga',
                trigger: 'blur',
              },
            ],
          },
            ...
            const { data } = await this.$apollo.mutate({
              ...
              update: (store, { data: { karyawanKeluargaCreate } }) => {
                ...
                const merge = _.merge(
                  ...
                  _.merge(
                    data.karyawanDetail.keluarga,
                    karyawanKeluargaCreate.keluarga,
                  ),
                );
                store.writeQuery({
                  query: KARYAWAN_KELUARGA,
            ...
            const { data } = await this.$apollo.mutate({
              ...
              update: (store, { data: { karyawanKeluargaUpdate } }) => {
                ...
                const merge = _.merge(
                  ...
                  _.merge(
                    data.karyawanDetail.keluarga,
                    karyawanKeluargaUpdate.keluarga,
                  ),
                );
                store.writeQuery({
                  query: KARYAWAN_KELUARGA,
